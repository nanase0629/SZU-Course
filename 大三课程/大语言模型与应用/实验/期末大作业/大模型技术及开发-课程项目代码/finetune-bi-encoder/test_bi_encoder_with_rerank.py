...
# 评估函数：bi-encoder召回top20，reranker重排top20。若正样本未进top20，则rank记为其在全体中的原始排名（bi-encoder分数排序后的位置+1），而不是9999。
def evaluate_with_rerank(bi_model_path, rerank_model_path, questions, answers):
    ...
    for idx, (q, q_emb) in enumerate(zip(questions, question_embs)):
        ...
        if idx in rerank_indices:
            ...
            # 正样本未进bi-encoder top50，记其在全体中的原始bi-encoder排名
            all_indices = torch.argsort(scores, descending=True).tolist()
            orig_rank = all_indices.index(idx)
...
if __name__ == '__main__':
    ...
    print('===== Bi-encoder + Reranker 评测 =====')
    metrics = evaluate_with_rerank(bi_model_path, rerank_model_path, questions, answers)
    print('Bi-encoder + Reranker 指标:', metrics)
    # 保存对比结果到result_compare.json
    result = {
        'bi-encoder+reranker(finetuned)': metrics
    }
