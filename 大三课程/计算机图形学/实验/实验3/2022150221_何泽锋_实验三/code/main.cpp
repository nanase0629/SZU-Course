...
void init()
{
	...
	// 设置光源位置
	light->setTranslation(glm::vec3(0.0, 0.0, 1.0));
	...
	plane->setScale(glm::vec3(6, 6, 3));

	plane->setAmbient(glm::vec4(0.6, 0.8, 0.0, 1.0)); // 环境光
	plane->setDiffuse(glm::vec4(0., 0., 0., 1.0)); // 漫反射
	plane->setSpecular(glm::vec4(0., 0., 0., 1.0)); // 镜面反射
...
void display()
{
	...
	// 绘制影子
	glm::mat4 shadow = light->getShadowProjectionMatrix();
	// 传递投影变换矩阵
	modelMatrix = shadow * modelMatrix;
...
int main(int argc, char **argv)
{
	...
	GLFWwindow* mainwindow = glfwCreateWindow(600, 600, "2022150221_何泽锋_实验3", NULL, NULL);
	if (mainwindow == NULL)
	{
