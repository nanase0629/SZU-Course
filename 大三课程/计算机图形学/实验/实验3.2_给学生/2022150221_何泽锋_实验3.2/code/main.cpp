...
void display()
{
	...
	// 计算阴影的模型变换矩阵。
	glm::mat4 shadow({ -light_position.y,light_position.x,0,0 }, { 0, 0, 0, 0 }, { 0, light_position.z, -light_position.y, 0 }, { 0, 1, 0, -light_position.y });
	shadow = glm::transpose(shadow);
	// 传递 isShadow 变量。
	// 传递 unifrom 关键字的矩阵数据。
...
int main(int argc, char **argv)
{
	...
	GLFWwindow* mainwindow = glfwCreateWindow(600, 600, "2022150221_何泽锋_实验3.2", NULL, NULL);
	if (mainwindow == NULL)
	{
